...
pub mod vault {
    ...
    // pub fn withdraw(ctx: Context<DepositOrWithdrawAccs>) -> Result<()> {
    //     Ok(())
    // }
...
pub struct InitializeAccs<'info> {
    ...
    #[account(init, payer = user, seeds = [b"vault_state", user.key().as_ref()], bump, space = VaultState::INIT_SPACE)]
...
pub struct DepositOrWithdrawAccs<'info> {
    ...
    #[account(seeds = [b"vault_state", user.key().as_ref()], bump)]
    pub vault_state: Account<'info, VaultState>,
